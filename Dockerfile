...
# SET BUILD PROFILE FOR AOT COMPILE
ARG EUREKA_CLIENT_ENABLED=false
ARG SPRING_CLOUD_DISCOVERY_ENABLED=false
ARG SPRING_CLOUD_DISCOVERY_ENABLED=false
ARG SPRING_CLOUD_KUBERNETES_DISCOVERY_ENABLED=false

ENV EUREKA_CLIENT_ENABLED=${EUREKA_CLIENT_ENABLED}
ENV SPRING_CLOUD_DISCOVERY_ENABLED=${SPRING_CLOUD_DISCOVERY_ENABLED}
ENV SPRING_CLOUD_DISCOVERY_ENABLED=${SPRING_CLOUD_DISCOVERY_ENABLED}
ENV SPRING_CLOUD_KUBERNETES_DISCOVERY_ENABLED=${SPRING_CLOUD_KUBERNETES_DISCOVERY_ENABLED}
...
RUN set -eux; \
    ...
    ./mvnw --batch-mode \
      -Pnative \
      -Dbuild.target=${ARCH} \
      -Deureka.client.enabled=${EUREKA_CLIENT_ENABLED} \
      -Dspring.cloud.discovery.enabled=${SPRING_CLOUD_DISCOVERY_ENABLED} \
      -Dspring.cloud.kubernetes.discovery.enabled=${SPRING_CLOUD_KUBERNETES_DISCOVERY_ENABLED} \
