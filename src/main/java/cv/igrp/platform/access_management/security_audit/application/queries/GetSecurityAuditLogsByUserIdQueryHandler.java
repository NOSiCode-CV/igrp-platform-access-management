...
@Component
public class GetSecurityAuditLogsByUserIdQueryHandler implements QueryHandler<GetSecurityAuditLogsByUserIdQuery, ResponseEntity<Page<SecurityAuditLogDTO>>> {

    private final SecurityAuditLogRepository repository;
    private static final DateTimeFormatter FORMATTER = DateTimeFormatter.ISO_LOCAL_DATE_TIME;

    public GetSecurityAuditLogsByUserIdQueryHandler(SecurityAuditLogRepository repository) {
        this.repository = repository;
    }

    @Override
    @IgrpQueryHandler
    public ResponseEntity<Page<SecurityAuditLogDTO>> handle(GetSecurityAuditLogsByUserIdQuery query) {
        Page<SecurityAuditLogDTO> logs = repository.findByUserId(query.getUserId(), query.getPageable())
                .map(this::toDTO);
