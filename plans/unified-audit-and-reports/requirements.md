...
- **R7.5** CSV is RFC 4180: `,` separator, `CRLF` line ends, fields containing a separator/quote/line break are quoted with embedded quotes doubled. Written UTF-8 with a BOM so Excel reads accented names correctly on double-click, and served as `text/csv;charset=UTF-8` — CSV declares no encoding in-band, so the charset must be on the wire (and on the stored object's content type). CSV touches no disk and no workbook model.
...
- **R7.12** A storage call must fail fast rather than block indefinitely. The MinIO SDK defaults connect/write/read to **5 minutes**, so an unreachable endpoint pins the calling HTTP worker for that long per upload — enough concurrent uploads exhaust the Tomcat pool and a storage outage escalates into an API outage. A hang is also not an exception, so R7.9's "record only after a successful upload" error path never runs. Timeouts are bounded via `igrp.storage.timeout.{connect,write,read}` (defaults 10s/60s/60s, per socket operation so a slow large upload is not cut off). Applies to the MinIO provider; the S3 provider builds an immutable client and is not yet covered.
