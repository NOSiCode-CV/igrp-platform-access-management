...
**Goal:** 9 download endpoints (3 reports × 3 formats), plus archiving a generated report to object storage with a retrievable DB record.
...
1. **Add dependencies** to `pom.xml`:
   ...
   <!-- POI 5.3.0 needs BoundedInputStream.builder() (>= 2.16); pin over the
        older transitive 2.14.0 or Excel fails at runtime. -->
   <dependency>
   ...
   ```
   Note: `openpdf-core-legacy` does not exist on Maven Central. The umbrella `openpdf` artifact at 2.0.x is the one that ships the legacy `com.lowagie.text` package.

2. **`ReportColumnDefinition`** value object (label + extractor), and **`ReportDescriptor`** bundling a report's columns, title key and filename prefix so exporters can be driven generically over `T`.

3. **`ExcelReportExporter`** — `SXSSFWorkbook` (streaming, 100-row window) when a writable scratch directory exists, else in-memory `XSSFWorkbook` (R7.6). `ExportTempFileConfig` probes `igrp.audit.export.temp-dir` once at startup and publishes `ExcelTempDirStatus`; the workbook type is chosen before any byte reaches the response.

4. **`PdfReportExporter`** — OpenPDF `PdfWriter` + `PdfPTable`, landscape. First page carries the header text ("Audit Report generated on YYYY-MM-DD") from `MessageSource`.

5. **`CsvReportExporter`** — RFC 4180 straight to the `OutputStream`, UTF-8 with BOM. No temp files, no workbook model.

6. **`ReportRenderer`** — resolves format → exporter and owns document title/filename, shared by the download and archive paths so a report looks identical either way.

7. **9 download methods** on `AuditReportsController`:
   ```
...
8. **Content-Disposition**: `attachment; filename="audit-report-YYYY-MM-DD.xlsx"` (etc.).

9. **Archive** (R7.7–R7.11):
   - `V13_1__create_audit_report_file.sql` → `t_audit_report_file` (`file_path`, report type, format, file name, content type, size, row count, filters JSON, period, generated by/at).
   - `AuditReportFileEntity` + `AuditReportFileRepository` + `AuditReportFileDTO`; `ReportType` / `ReportFormat` enums.
   - `AuditReportArchiveService` — uploads via `StorageService` directly (the bytes are server-generated, so `UploadFileCommandHandler`'s `MultipartFile` contract does not fit), keyed `private/audit-reports/{user}/{uuid}_{name}`; persists the record **only after** the upload succeeds.
   - `AuditReportArchiveController` — `POST /api/auth/reports/{audit,access,settings}/archive?format=` and `GET /api/auth/reports/archives` (via `GetAuditReportFilesQuery`).

10. **Deployment**: mount a writable `/tmp` (emptyDir in `k8s/deployment.yaml`) so Excel takes the streaming path.

11. **`.igrpstudio` JSON**: `audit/controllers/AuditReportArchiveController.json`, `audit/dto/AuditReportFileDTO.json`, `audit/models/AuditReportFileEntity.json`, `audit/enum/{ReportType,ReportFormat}.json`, plus the csv/archive actions on `AuditReportsController.json`.

### Tests

- Golden-file tests: generate each format against a fixed 5-row seed → reopen with POI / OpenPDF `PdfTextExtractor` / string assertions and check header + cells.
- Excel golden-file runs against **both** paths (SXSSF and the in-memory fallback).
- Archive service: key convention (`private/audit-reports/...`), per-format content types, and **no record persisted when the upload fails**.
- Load test (manual, not CI): 100k rows → assert memory + timing per §2.1 N2/N3, on the **download** path (the archive path is exempt — see N2).

### Deliverable

Nine working download endpoints and a report archive: `POST .../archive?format=` stores
the document under `private/audit-reports/` and records it in `t_audit_report_file`;
`GET .../archives` lists them; the frontend turns a record's `filePath` into a link
through the existing `GET /api/files/url`. Excel exports work whether or not the
runtime has a writable scratch directory, streaming when it does.
...
1. Add `cv.igrp.platform.access.client.api.AuditReportsApi`:
   ...
   - `InputStream get<Report>Pdf(filters)` × 3 reports
   - `InputStream get<Report>Xlsx(filters)` × 3 reports
   - `InputStream get<Report>Csv(filters)` × 3 reports
   - `AuditReportFileDTO archive<Report>(format, filters)` × 3 reports
   - `Page<AuditReportFileDTO> listArchives(reportType, format, generatedBy, pageable)`
2. Add DTOs + enums to `cv.igrp.platform.access.client.model.reports.*` — including `AuditReportFileDTO`, `ReportType`, `ReportFormat`.
   Note the archive methods return the **record**, not the bytes: callers take `filePath` and resolve it through the existing files API (`GET /api/files/url`) to obtain a short-lived presigned link (R7.11).
...
| 4 | 4–5h | Med — POI needs a writable scratch dir (SXSSF makes its temp file at sheet creation, so a missing one breaks .xlsx at every size); archive adds a table + storage round-trip |
