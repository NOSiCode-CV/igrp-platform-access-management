...
|---------|----------|------------|
| `technical` | CRUD-heavy APIs, straightforward service/repository flow | Low |
| `domain-lite` | Feature modules need CQRS/event split without full DDD overhead | Medium |
| `domain` | Complex domains, explicit command/query/event flows, strict boundaries | High |
...
- Spring Modulith for modular boundary verification when domain boundaries are strict
...
- `technical`: use conventional controllers/services/repositories/models/dto/constants packages
- `domain-lite`: split each module into `application`, `domain`, and `infrastructure` with lean CQRS
- `domain`: enforce full application/domain/infrastructure separation with explicit contracts
