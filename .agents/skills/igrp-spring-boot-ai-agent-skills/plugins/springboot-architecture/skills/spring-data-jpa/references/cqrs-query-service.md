...
Separate read operations from write operations for iGRP `domain-lite` and `domain` architectures.
...
## iGRP package placement

- `domain-lite`:
  - write contracts in `<module>.domain.repository`
  - query services in `<module>.application.queries`
  - response view models in `<module>.application.response`
- `domain`:
  - write contracts in `<module>.domain.repository`
  - query orchestration in `<module>.application.query`
  - infrastructure SQL/JPA adapters in `<module>.infrastructure.db`
